...
- 生成时间：`2026-08-31T13:03:59`
- 平台地址：`http://127.0.0.1:57292`
- 通过率：`100.0%`（15/15）
- 页面功能覆盖率：`100.0%`（24/24）
- 断言数：`11`
...
|---|---|---:|---:|---:|---|
| E2E01 | 页面导航与订单明细抽样展示 | PASS | 3 | 3430.16 | - |
| E2E02 | 上游业务表页面编辑保存回读 | PASS | 1 | 12767.49 | - |
| E2E03 | 场景配置传递到模型入参并高亮 | PASS | 2 | 12407.96 | - |
| E2E04 | 求解流水线进度与成本结构可视化 | PASS | 1 | 6415.07 | - |
| E2E05 | 审批通过与执行发布闭环 | PASS | 0 | 8476.88 | - |
| E2E06 | 运行历史报告与回放 | PASS | 0 | 7736.58 | - |
| E2E07 | 数据血缘与约束解释页面导航 | PASS | 0 | 9646.53 | - |
| E2E08 | 压力测试与容量评估页面 | PASS | 0 | 14275.2 | - |
| E2E09 | 上游数据二级页面覆盖 | PASS | 1 | 4231.83 | - |
| E2E10 | 场景配置保存审计与权限 | PASS | 1 | 6436.56 | - |
| E2E11 | 四模型入参导航与重新生成 | PASS | 1 | 8487.32 | - |
| E2E12 | 结果页面板、三方案对比与报告导出 | PASS | 0 | 7661.15 | - |
| E2E13 | 角色权限隔离页面校验 | PASS | 0 | 7841.83 | - |
| E2E14 | 审批驳回路径与发布拦截 | PASS | 0 | 5845.19 | - |
| E2E15 | 血缘多视图与四模型约束覆盖 | PASS | 1 | 9419.94 | - |
...
```json
{
  "approval_status": "当前运行记录：cb6aa0ad，已批准，当前角色：管理员",
  "publish_status": "已发布：EXEC-20260831-cb6aa0ad-5cc74a，下游目标 4 个"
}
```
...
{
  "history_rows": 20,
  "replay_status": "已回放运行记录 cb6aa0ad，未新增运行记录。"
}
```
...
{
  "playbook_count": 3,
  "config_status": "已保存到：starrocks://127.0.0.1:9030/cplex_poc.platform_playbooks",
  "audit_first_row": "2026/8/29 07:35:13\tbaseline\tplatform_ui\tsla_extra_days: 1 -> 0",
...
```json
{
  ...
  "人员排班": "model_name\tstaff_scheduling_soft\t实际调用的排班模型\t-\nemployees\tAlice, Bob, Carol, David\t可排班员工集合\t-\ndays\tMon, Tue, Wed, Thu, Fri, Sat, Sun\t排班周期\t-\ntotal_required_staff\t19\t总人班需求\t旺季排班开关影响人力需求\nmax_shifts_per_employee\t3\t每人最多班次数\t旺季排班开关影响人力需求\nsoft_constraints\ttrue\t是否允许缺口并用罚分兜底\t软约束开关影响缺口处理"
}
```
...
{
  "compare_rows": 10,
  "export_status": "已导出：python/reports/platform_report_20260831_130335_peak.md",
...
```json
{
  "approval_status": "当前运行记录：da5a9dc6，已驳回，当前角色：管理员",
