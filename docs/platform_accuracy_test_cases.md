...
- `E2E01` - `E2E16`：使用 Playwright 启动真实 Chromium 浏览器，模拟用户点击页面、切换导航、编辑表格、运行模型、审批发布、权限隔离、报告导出、回放记录和四步准确性链路。
...
目的：模拟用户从上游数据开始，经过场景配置、模型入参到求解结果，验证页面展示的数据传递和结果拆解准确。

操作：浏览器访问 `/upstream`；进入“业务表编辑”读取市场需求和天气风险线路数；访问 `/config` 选择基准方案并设置确定的需求倍率、空运容量、缺口罚分和 SLA 放宽天数；点击“保存临时参数并运行”；访问 `/inputs` 查看仓网模型入参；访问 `/results` 查看求解结果和成本结构。

断言：模型入参 `total_demand` 等于页面上游市场需求合计乘以场景需求倍率后的取整值；模型入参 `weather_impacted_lanes` 等于页面上游天气线路数；求解结果综合成本大于 0；综合成本等于成本结构中仓网、补货、排班和服务组合成本之和。

通过标准：上游数据、场景参数、模型入参和求解结果四个页面之间的关键数字可以闭环核对。
