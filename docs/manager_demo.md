...
- 上游数据：解释 OMS/WMS/TMS/HR 数据如何接入，支持 1000 条订单明细，并做数据结构、容量和 SLA 质量校验。
- 场景配置：选择基准、旺季或稳健方案，也可以调整需求倍率、空运能力、缺口罚分或 SLA，并查看这些参数影响哪些模型和约束。
- 模型入参：展示进入 CPLEX 的仓网、补货、服务和排班数据，并高亮受场景参数影响的入参字段。
- 求解结果：用求解流水线、醒目的成本结构、决策闭环状态、平台推荐方案和三方案差异矩阵把 objective value 翻译成综合成本、缺口、行动建议、执行计划、审批分层和差异来源，保存/回放运行记录，并一键导出演示报告。
- 数据血缘：用按模型分组的可视化泳道图展示上游表、场景参数、转换函数、CPLEX 入参、求解器和结果字段。
- 约束解释：用业务语言解释每个模型的变量、目标函数、核心约束和输出字段。
- 页面状态：六个页面共享当前方案和临时参数，场景配置页修改后，模型入参和求解结果页会沿用同一组参数。
- 数据层：方案配置来自 `python/data/platform_poc_data.json`，页面用业务参数表单承接配置，不再暴露 JSON 编辑器。
