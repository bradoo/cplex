...
- 基准运营方案：说明模型可以给日常运营方案。
- 旺季高峰方案：说明需求上涨后，模型会暴露缺口和审批风险。
- 稳健服务方案：说明可以通过更高服务能力和更宽 SLA 做方案取舍。
- 管理驾驶舱：把 objective value 翻译成综合成本、缺口、行动建议和审批分层。
- 参数区：现场调整需求倍率、空运能力、缺口罚分或 SLA，展示 what-if 分析。
- 三方案对比：展示同一批模型如何支持管理层在成本、缺口和审批风险之间选择。
- 数据层：方案入参来自 `python/data/platform_poc_data.json`，页面会展示当前进入模型的参数，也可以现场修改 JSON 并保存后重新求解。
