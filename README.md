...
```text
1. 上游数据：解释 OMS/WMS/TMS/HR 数据如何接入，支持 1000 条订单明细，并做数据质量校验
2. 场景配置：选择基准、旺季或稳健方案，并查看参数影响的模型与约束
3. 模型入参：展示进入 CPLEX 的仓网、补货、服务和排班数据，并高亮受场景参数影响的入参
4. 求解结果：用求解流水线、醒目的成本结构、决策闭环状态、平台推荐方案和三方案差异矩阵解释 KPI、行动建议、执行计划、审批分层和方案差异，保存/回放运行记录，并一键导出演示报告
